...
// make wouldn't compile add void prototypes here
// moving prototypes after struct fixed compilation issues 
void solveNodeA(struct node *curr, char **inputs, char **outputs, int nIn, int nOut, int rowNum, short **table, struct node *start);
...
struct node * createNodeA (char * name, short type, char * A, char * B, int rowNum) {
	...
	// all unneeded info becomes null
	ptr->inArray = NULL;
 	ptr->decOut = NULL;
 	ptr->selectors = NULL;
 	ptr->multOut=NULL;
 	...
 	// The current output to 2, so know its wrong if it prints
...
struct node * createNodeB (char * name, short type, char * A, int rowNum) {
	...
	// copy in the name of what is produced (output/temp)
	for (size =0; name[size]!='\0'; size++) {
	}size++;
...
// create the truth table and initializes the input columns in the table
short ** createT (int numI, int numO, int rows) {
	...
	// input input colums to the correct order of 1s and 0s
	for (i=2; i<rows; i++) {
		for (j = (numI + numO -1); j>=0; j--) {
...
// prints the table with or without headers
...
void solveNodeB (struct node * ptr, char ** inputs, char ** outputs, int nIn, int nOut, int rowNum, short ** table, struct node * start) {
	...
	// if the inputs for the instruction are not inputs, check to see if they are solved, if not solve them
	if (columnA == -1) {
		struct node * curr = start;
		while (curr != NULL) {
			// if already solved, pull out its outputs
...
// Takes the list of math instructions and evaluates each of them
...
int main (int argc, char ** argv) {
	...
	// Figures out if read from stdin, or if you read from file
